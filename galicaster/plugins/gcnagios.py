...
def init():
    try:
        ...
        make_plugin_path()
        plugin_path = conf.conf_folder + GC_NAGIOS_PLUGINS_DIR
        gcn = GCNagios(plugin_path, plugin_template)
        dispatcher.connect('recorder-error', gcn.nagios_gst_error)
        dispatcher.connect('galicaster-init', gcn.nagios_default_state)
...
def make_plugin_path():
    ...
    if not os.path.exists(conf.conf_folder + GC_NAGIOS_PLUGINS_DIR):
        os.makedirs(conf.conf_folder + GC_NAGIOS_PLUGINS_DIR)
